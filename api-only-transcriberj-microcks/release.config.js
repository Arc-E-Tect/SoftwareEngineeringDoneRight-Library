...
module.exports = {
    ...
    // other modules in this repo (sedr-library and hexagonal-spring-rules share the unscoped
    // "v${version}" tag format). A "api-only-transcriberj-microcks-v0.0.0" tag is seeded as the baseline, and
    // ./first-release-patch-analyzer.js forces the release type to "patch" whenever no prior
    // release exists for this tag namespace, so the first real release computes as exactly 0.0.1
    // regardless of the triggering commit's type.
    tagFormat: "api-only-transcriberj-microcks-v${version}",
    plugins: [
        ["./first-release-patch-analyzer", {
            preset: 'angular',
            releaseRules: [
