 ...
 * lifecycle - declared, implemented, verified - keyed by {@link EndpointFingerprint} rather than
 * by which plugin observed it, so that Shadow, Mirage, and Doppelganger API Detector can all
 * advance the same record from their own, partial view of the endpoint.
 *
 ...
 *                       this endpoint, or {@code null} when no plugin has observed an
 *                       implementation for it yet (e.g. it is only known as declared so far)
 ...
 * @param implementedAt  when the endpoint was first observed as implemented by a
 *                       {@code @RestController} method, or {@code null} if it never has been
