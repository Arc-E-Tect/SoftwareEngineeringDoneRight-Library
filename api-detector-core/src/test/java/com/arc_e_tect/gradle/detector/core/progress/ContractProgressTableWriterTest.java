...
class ContractProgressTableWriterTest {
    ...
    void reportsNaWhenNoTimestampsPresent() {
        ContractProgressRecord record = new ContractProgressRecord(
                "fp1", HttpVerb.GET, "/orders/{id}", null, null, null, null, null, null, null);
    ...
    void countsRecordsWithinLast7And30Days() {
        ...
        Map<String, ContractProgressRecord> history = Map.of(
                "fp1", new ContractProgressRecord(
                        ...
                        recentlyDeclared, recentlyImplemented, null, recentlyVerified, recentlyDeclared, null),
                "fp2", new ContractProgressRecord(
                        "fp2", HttpVerb.GET, "/orders/{id}", "com.example.OrderController",
                        longAgo, longAgo, null, longAgo, longAgo, null));
    ...
    @Test
    @DisplayName("counts records stubbed within the last 7 and 30 days")
    void countsRecordsStubbedWithinLast7And30Days() {
        Instant recentlyStubbed = Instant.now().minus(Duration.ofDays(3));
        Instant longAgo = Instant.now().minus(Duration.ofDays(40));
        Map<String, ContractProgressRecord> history = Map.of(
                "fp1", new ContractProgressRecord(
                        "fp1", HttpVerb.GET, "/orders", "(mappings)",
                        recentlyStubbed, null, recentlyStubbed, null, recentlyStubbed, null),
                "fp2", new ContractProgressRecord(
                        "fp2", HttpVerb.GET, "/orders/{id}", "(mappings)",
                        longAgo, null, longAgo, null, longAgo, null));

        String output = render(history);

        assertThat(output)
                .contains("| Stubbed in the last 7 days" + System.lineSeparator() + "| 1")
                .contains("| Stubbed in the last 30 days" + System.lineSeparator() + "| 1");
    }

    @Test
    @DisplayName("reports zero verified and zero stubbed when neither has ever been observed, e.g. Shadow/Doppelganger history")
    void reportsZeroVerifiedAndZeroStubbedWhenNeverObserved() {
        ContractProgressRecord record = new ContractProgressRecord(
                "fp1", HttpVerb.GET, "/orders", "com.example.OrderController",
                Instant.now(), Instant.now(), null, null, Instant.now(), null);
        ...
        assertThat(output)
                ...
                .contains("| Stubbed in the last 7 days" + System.lineSeparator() + "| 0")
                .contains("| Stubbed in the last 30 days" + System.lineSeparator() + "| 0");
    }
